...
{
  temp_df <-
    fsn(effects_algae[1:i, ]$yi, effects_algae[1:i, ]$vi, type = "Rosenthal")
  temp_df$fsnum
  failsafe_one_run <-
    (temp_df$fsnum) / (5 * length(effects_algae[1:i, ]$yi) + 10)
...
# Algae graph
gg_algae <-
  ggplot(effects_algae, aes(x = failsaferatio, y = order)) + geom_point(aes(colour = failsaferatio > 1))
gg_algae
gg_algae <-
  gg_algae + scale_colour_manual(values = setNames(c('red', 'black'), c(T, F)))
gg_algae
gg_algae <-
  gg_algae + scale_y_continuous(
    trans = "reverse",
...
effects_aquatic_plants$fsr <-
  rep(NA, length(effects_aquatic_plants$code))
effects_aquatic_plants
for (i in 1:length(effects_aquatic_plants$code))
{
  temp_df <-
    fsn(effects_aquatic_plants[1:i, ]$yi,
        effects_aquatic_plants[1:i, ]$vi,
        type = "Rosenthal")
  temp_df$fsnum
  failsafe_one_run <-
    (temp_df$fsnum) / (5 * length(effects_aquatic_plants[1:i, ]$yi) + 10)
...
# Aquatic graph
gg_aquatic <-
  ggplot(effects_aquatic_plants, aes(x = failsaferatio, y = order)) + geom_point(aes(colour = failsaferatio > 1))
gg_aquatic
gg_aquatic <-
  gg_aquatic + scale_colour_manual(values = setNames(c('red', 'black'), c(T, F)))
gg_aquatic
gg_aquatic <-
  gg_aquatic + scale_y_continuous(
    trans = "reverse",
...
{
  temp_df <-
    fsn(effects_crust[1:i, ]$yi, effects_crust[1:i, ]$vi, type = "Rosenthal")
  temp_df$fsnum
  failsafe_one_run <-
    (temp_df$fsnum) / (5 * length(effects_crust[1:i, ]$yi) + 10)
...
# Crust graph
gg_crust <-
  ggplot(effects_crust, aes(x = failsaferatio, y = order)) + geom_point(aes(colour = failsaferatio > 1))
gg_crust
gg_crust <-
  gg_crust + scale_colour_manual(values = setNames(c('red', 'black'), c(T, F)))
gg_crust
gg_crust <-
  gg_crust + scale_y_continuous(
    trans = "reverse",
...
{
  temp_df <-
    fsn(effects_fish[1:i, ]$yi, effects_fish[1:i, ]$vi, type = "Rosenthal")
  temp_df$fsnum
  failsafe_one_run <-
    (temp_df$fsnum) / (5 * length(effects_fish[1:i, ]$yi) + 10)
...
# Fish graph
gg_fish <-
  ggplot(effects_fish, aes(x = failsaferatio, y = order)) + geom_point(aes(colour = failsaferatio > 1))
gg_fish
gg_fish <-
  gg_fish + scale_colour_manual(values = setNames(c('red', 'black'), c(T, F)))
gg_fish
gg_fish <-
  gg_fish + scale_y_continuous(
    trans = "reverse",
...
{
  temp_df <-
    fsn(effects_grass[1:i, ]$yi, effects_grass[1:i, ]$vi, type = "Rosenthal")
  temp_df$fsnum
  failsafe_one_run <-
    (temp_df$fsnum) / (5 * length(effects_grass[1:i, ]$yi) + 10)
...
# Grass graph
gg_grass <-
  ggplot(effects_grass, aes(x = failsaferatio, y = order)) + geom_point(aes(colour = failsaferatio > 1))
gg_grass
gg_grass <-
  gg_grass + scale_colour_manual(values = setNames(c('red', 'black'), c(T, F)))
gg_grass
gg_grass <-
  gg_grass + scale_y_continuous(
    trans = "reverse",
...
{
  temp_df <-
    fsn(effects_herb[1:i, ]$yi, effects_herb[1:i, ]$vi, type = "Rosenthal")
  temp_df$fsnum
  failsafe_one_run <-
    (temp_df$fsnum) / (5 * length(effects_herb[1:i, ]$yi) + 10)
...
# Herb graph
gg_herb <-
  ggplot(effects_herb, aes(x = failsaferatio, y = order)) + geom_point(aes(colour = failsaferatio > 1))
gg_herb
gg_herb <-
  gg_herb + scale_colour_manual(values = setNames(c('red', 'black'), c(T, F)))
gg_herb
gg_herb <-
  gg_herb + scale_y_continuous(
    trans = "reverse",
...
{
  temp_df <-
    fsn(effects_insect[1:i, ]$yi, effects_insect[1:i, ]$vi, type = "Rosenthal")
  temp_df$fsnum
  failsafe_one_run <-
    (temp_df$fsnum) / (5 * length(effects_insect[1:i, ]$yi) + 10)
...
# insect graph
gg_insect <-
  ggplot(effects_insect, aes(x = failsaferatio, y = order)) + geom_point(aes(colour = failsaferatio > 1))
gg_insect
gg_insect <-
  gg_insect + scale_colour_manual(values = setNames(c('red', 'black'), c(T, F)))
gg_insect
gg_insect <-
  gg_insect + scale_y_continuous(
    trans = "reverse",
...
{
  temp_df <-
    fsn(effects_mammal[1:i, ]$yi, effects_mammal[1:i, ]$vi, type = "Rosenthal")
  temp_df$fsnum
  failsafe_one_run <-
    (temp_df$fsnum) / (5 * length(effects_mammal[1:i, ]$yi) + 10)
...
# mammal graph
gg_mammal <-
  ggplot(effects_mammal, aes(x = failsaferatio, y = order)) + geom_point(aes(colour = failsaferatio > 1))
gg_mammal
gg_mammal <-
  gg_mammal + scale_colour_manual(values = setNames(c('red', 'black'), c(T, F)))
gg_mammal
gg_mammal <-
  gg_mammal + scale_y_continuous(
    trans = "reverse",
...
{
  temp_df <-
    fsn(effects_molluscs[1:i, ]$yi, effects_molluscs[1:i, ]$vi, type = "Rosenthal")
  temp_df$fsnum
  failsafe_one_run <-
    (temp_df$fsnum) / (5 * length(effects_molluscs[1:i, ]$yi) + 10)
...
# moll graph
gg_moll <-
  ggplot(effects_molluscs, aes(x = failsaferatio, y = order)) + geom_point(aes(colour = failsaferatio > 1))
gg_moll
gg_moll <-
  gg_moll + scale_colour_manual(values = setNames(c('red', 'black'), c(T, F)))
gg_moll
gg_moll <-
  gg_moll + scale_y_continuous(
    trans = "reverse",
...
{
  temp_df <-
    fsn(effects_tree[1:i, ]$yi, effects_tree[1:i, ]$vi, type = "Rosenthal")
  temp_df$fsnum
  failsafe_one_run <-
    (temp_df$fsnum) / (5 * length(effects_tree[1:i, ]$yi) + 10)
...
# Tree graph
gg_tree <-
  ggplot(effects_tree, aes(x = failsaferatio, y = order)) + geom_point(aes(colour = failsaferatio > 1))
gg_tree
gg_tree <-
  gg_tree + scale_colour_manual(values = setNames(c('red', 'black'), c(T, F)))
gg_tree
gg_tree <-
  gg_tree + scale_y_continuous(
    trans = "reverse",
